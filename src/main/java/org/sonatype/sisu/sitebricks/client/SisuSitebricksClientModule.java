...
import org.sonatype.sisu.sitebricks.client.internal.RestClientFactoryProvider;
...
public class SisuSitebricksClientModule
{
    ...
    public <S> Module build( final Class<S> serviceInterface )
    {
        return new AbstractModule()
        {
            ...
            {
                final Provider<RestClientFactory<S>> provider =
                    new RestClientFactoryProvider<S>( serviceInterface, bindings );
                bind( (Key)Key.get( Types.newParameterizedType(
                    RestClientFactory.class, serviceInterface ) ) ).toProvider( provider );
            }
        };
    }
        ...
        public SisuSitebricksClientModule to( final Object value )
        {
            if ( value == null )
            {
                throw new IllegalArgumentException(
                    String.format( "Value binded to [%s] variable cannot be null", name ) );
            }
